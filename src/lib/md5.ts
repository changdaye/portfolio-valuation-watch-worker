...
function md5Block(input: string) {
...
function toHex(x: number) {
  let output = '';
  for (let i = 0; i < 4; i += 1) {
    output += ((x >> (i * 8 + 4)) & 0x0f).toString(16);
    output += ((x >> (i * 8)) & 0x0f).toString(16);
  }
  return output;
}

export function md5(input: string): string {
  return md5String(unescape(encodeURIComponent(input))).map(toHex).join('');
}
